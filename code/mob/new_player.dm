...
mob/new_player/ex_act(severity)
	return

mob/new_player/disposing()
	mobs.Remove(src)
	if (mind)
		if (mind.current == src)
			mind.current = null
...
mob/new_player/proc/create_character(var/datum/job/J, var/allow_late_antagonist = 0)
...
mob/new_player/proc/makebad(var/mob/living/carbon/human/traitormob, type)
...
mob/new_player/verb/declare_ready()
	set hidden = 1
	set name = ".ready"
...
mob/new_player/verb/cancel_ready()
	set hidden = 1
...
mob/new_player/verb/observe_round()
	set hidden = 1
