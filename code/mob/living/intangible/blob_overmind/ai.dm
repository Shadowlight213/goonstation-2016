...
/mob/living/intangible/blob_overmind/ai/New()
	ai_id = src.next_id
	src.next_id++
...
/mob/living/intangible/blob_overmind/ai/proc/priority(var/obj/O)
	if (!O.density)
		return 0
...
/mob/living/intangible/blob_overmind/ai/proc/is_bottleneck(var/turf/T)
...
/mob/living/intangible/blob_overmind/ai/proc/update_lists(var/turf/ST)
	if (ST in open)
		open -= ST
...
/mob/living/intangible/blob_overmind/ai/proc/get_nearby_convertable_blob(var/turf/T)
	for (var/obj/blob/B in orange(1, T))
		if (B.type == /obj/blob)
			return B
	return null

/mob/living/intangible/blob_overmind/ai/proc/evaluate_no_add(var/turf/C)
	if (!C)
		return -2
...
/mob/living/intangible/blob_overmind/ai/proc/evaluate(var/turf/C)
...
/mob/living/intangible/blob_overmind/ai/proc/has_adjacent_blob(var/turf/T)
	var/turf/N = locate(T.x, T.y + 1, T.z)
	var/turf/S = locate(T.x, T.y - 1, T.z)
	var/turf/E = locate(T.x + 1, T.y, T.z)
	var/turf/W = locate(T.x - 1, T.y, T.z)
...
/mob/living/intangible/blob_overmind/ai/proc/pick_deployment_location()
...
/mob/living/intangible/blob_overmind/ai/proc/check_viability(var/turf/start)
	var/list/closed = list()
...
/mob/living/intangible/blob_overmind/ai/Life(datum/controller/process/mobs/parent)
	if (..(parent))
		return 1
	if (client)
		return
...
/mob/living/intangible/blob_overmind/ai/proc/attack_now(var/turf/T)
	attack.last_used = 0 // cheat, to compensate for the loop's tick rate
	set_loc(T)
	attack.onUse(T)

/mob/living/intangible/blob_overmind/ai/proc/spread_to(var/turf/ST, var/is_calm)
	set_loc(ST)
	spread.onUse(ST)
...
/mob/living/intangible/blob_overmind/ai/proc/create_mitochondria_if_possible(var/turf/T)
...
/mob/living/intangible/blob_overmind/ai/proc/create_wall_if_possible(var/turf/T)
...
/mob/living/intangible/blob_overmind/ai/onBlobHit(var/obj/blob/B, var/mob/M)
...
/mob/living/intangible/blob_overmind/ai/onBlobDeath(var/obj/blob/B, var/mob/M)
	if (!prob(max(1, min(100, (2000 - 100 * get_dist(B, src)) / 13))))
		return
	attacker = M
