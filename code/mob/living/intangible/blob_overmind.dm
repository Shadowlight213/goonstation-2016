...
/mob/living/intangible/blob_overmind/proc/start_tutorial()
	if (tutorial)
		return
	tutorial = new(src)
...
/mob/living/intangible/blob_overmind/New()
	..()
...
/mob/living/intangible/blob_overmind/Move(NewLoc)
	if (tutorial)
...
/mob/living/intangible/blob_overmind/Life(datum/controller/process/mobs/parent)
	if (..(parent))
		return 1
...
/mob/living/intangible/blob_overmind/Stat()
	..()
	stat(null, " ")
	stat("--Blob--", " ")
...
/mob/living/intangible/blob_overmind/Login()
	..()
	src.update_buttons()
	client.show_popup_menus = 0

/mob/living/intangible/blob_overmind/Logout()
	..()
	if (src.last_client)
...
/mob/living/intangible/blob_overmind/MouseDrop()
	return

/mob/living/intangible/blob_overmind/MouseDrop_T()
	return

/mob/living/intangible/blob_overmind/meteorhit()
	return

/mob/living/intangible/blob_overmind/is_spacefaring()
	return 1

/mob/living/intangible/blob_overmind/click(atom/target, params)
...
/mob/living/intangible/blob_overmind/say_understands()
	return 1
/mob/living/intangible/blob_overmind/can_use_hands()
	return 0

/mob/living/intangible/blob_overmind/say(var/message)
	return ..(message)

/mob/living/intangible/blob_overmind/say_quote(var/text)
...
/mob/living/intangible/blob_overmind/proc/get_gen_rate()
	return base_gen_rate + gen_rate_bonus - gen_rate_used

/mob/living/intangible/blob_overmind/proc/onBlobHit(var/obj/blob/B, var/mob/M)
	return

/mob/living/intangible/blob_overmind/proc/onBlobDeath(var/obj/blob/B, var/mob/M)
	return

/mob/living/intangible/blob_overmind/proc/add_ability(var/ability_type)
...
/mob/living/intangible/blob_overmind/proc/add_upgrade(var/upgrade_type, var/skip_disabled = 0)
...
/mob/living/intangible/blob_overmind/proc/remove_ability(var/ability_type)
	if (!ispath(ability_type))
		return
...
/mob/living/intangible/blob_overmind/proc/get_ability(var/ability_type)
	if (!ispath(ability_type))
		return null
	for (var/datum/blob_ability/A in src.abilities)
		if (A.type == ability_type)
...
/mob/living/intangible/blob_overmind/proc/get_upgrade(var/upgrade_type)
	if (!ispath(upgrade_type))
		return null
...
/mob/living/intangible/blob_overmind/proc/update_buttons()
	if(!src.client)
		return
...
/mob/living/intangible/blob_overmind/proc/has_ability(var/ability_path)
	for (var/datum/blob_ability/B in src.abilities)
		if (B.type == ability_path)
			return 1
	return 0

/mob/living/intangible/blob_overmind/proc/has_upgrade(var/upgrade_path)
	for (var/datum/blob_upgrade/B in src.upgrades)
		if (B.type == upgrade_path)
			return 1
	return 0

/mob/living/intangible/blob_overmind/proc/BlobPointsBezierApproximation(var/t)
...
/mob/living/intangible/blob_overmind/proc/usePoints(var/amt, var/force = 1)
	if (bio_points < amt)
...
/mob/living/intangible/blob_overmind/proc/hasPoints(var/amt)
	for (var/Q in lipids)
		if (!istype(Q, /obj/blob/lipid))
			lipids -= Q
	return bio_points + lipids.len * 4 >= amt

/mob/living/intangible/blob_overmind/projCanHit(datum/projectile/P)
	return 0
...
/obj/screen/blob/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
...
/obj/screen/blob/clicked(parameters)
