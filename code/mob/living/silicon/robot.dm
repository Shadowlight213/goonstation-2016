...
/mob/living/silicon/robot/New(loc, var/obj/item/parts/robot_parts/robot_frame/frame = null, var/starter = 0, var/syndie = 0)
	hud = new(src)
	src.attach_hud(hud)
...
/mob/living/silicon/robot/drop_item_v()
	return
...
/mob/living/silicon/robot/update_cursor()
	if (src.client)
...
/mob/living/silicon/robot/emote(var/act, var/voluntary = 1)
	var/param = null
...
/mob/living/silicon/robot/choose_name(var/retries = 3)
	var/newname
...
/mob/living/silicon/robot/blob_act(var/power)
	if (src.stat != 2)
		var/Pshield = 0
...
/mob/living/silicon/robot/bullet_act(var/obj/projectile/P)
...
/mob/living/silicon/robot/emag_act(var/mob/user, var/obj/item/card/emag/E)
...
/mob/living/silicon/robot/emp_act()
	vision.noise(60)
...
/mob/living/silicon/robot/temperature_expose(null, temp, volume)
	var/Fshield = 0
...
/mob/living/silicon/robot/action(num)
	switch (num)
		if (1 to 4) // 4 will deselect the module
			swap_hand(num)

/mob/living/silicon/robot/swap_hand(var/switchto = 0)
...
/mob/living/silicon/robot/click(atom/target, params)
...
/mob/living/silicon/robot/get_equipped_ore_scoop()
...
/mob/living/silicon/robot/proc/uneq_slot(var/i)
	if (module_states[i])
...
/mob/living/silicon/robot/proc/activate_upgrade(obj/item/roboupgrade/upgrade)
	if(!upgrade) return
...
/mob/living/silicon/robot/proc/toggle_module_pack()
	if(weapon_lock)
...
/mob/living/silicon/robot/proc/spellopen()
	if (src.locked)
		locked = 0
	if (src.locking)
		src.locking = 0
...
/mob/living/silicon/robot/verb/cmd_toggle_lock()
	set category = "Robot Commands"
...
/mob/living/silicon/robot/proc/get_poweruse_count()
	if (src.cell)
		var/efficient = 0
...
/mob/living/silicon/robot/proc/update_canmove()
...
/mob/living/silicon/robot/proc/mainframe_check()
...
/mob/living/silicon/robot/proc/update_bodypart(var/part = "all")
...
/mob/living/silicon/robot/proc/update_appearance()
...
/mob/living/silicon/robot/proc/compborg_force_unequip(var/slot = 0)
	src.module_active = null
	switch(slot)
		if(1)
			uneq_slot(1)
...
/mob/living/silicon/robot/TakeDamage(zone, brute, burn)
	brute = max(brute, 0)
	burn = max(burn, 0)
	if (burn == 0 && brute == 0)
		return 0
...
/mob/living/silicon/robot/HealDamage(zone, brute, burn)
	brute = max(brute, 0)
	burn = max(burn, 0)
	if (burn == 0 && brute == 0)
		return 0
...
/mob/living/silicon/robot/get_brute_damage()
...
/mob/living/silicon/robot/get_burn_damage()
	if (!part_chest || !part_head)
		return 200
...
/mob/living/silicon/robot/get_valid_target_zones()
	return list("head", "chest", "l_leg", "r_leg", "l_arm", "r_arm")

/mob/living/silicon/robot/proc/compborg_lose_limb(var/obj/item/parts/robot_parts/part)
	if(!part) return
...
/mob/living/silicon/robot/proc/compborg_get_total_damage(var/sort = 0)
	var/tally = 0
...
/mob/living/silicon/robot/proc/compborg_take_critter_damage(var/zone = null, var/brute = 0, var/burn = 0)
