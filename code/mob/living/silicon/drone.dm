...
/obj/item/device/drone_control/attack_self(var/mob/user as mob)
	drone_list = list()
...
/mob/living/silicon/drone/examine()
	..()
	if (src.controller)
...
/mob/living/silicon/drone/movement_delay()
	var/tally = 0
...
/mob/living/silicon/drone/attackby(obj/item/W as obj, mob/user as mob)
...
/mob/living/silicon/drone/proc/take_damage(var/amount)
	if (!isnum(amount))
		return
...
/mob/living/silicon/drone/damage_blunt(var/amount)
	if (!isnum(amount) || amount <= 0)
		return
	take_damage(amount)

/mob/living/silicon/drone/damage_heat(var/amount)
	if (!isnum(amount) || amount <= 0)
		return
	take_damage(amount)

/mob/living/silicon/drone/swap_hand(var/switchto = 0)
	if (!isnum(switchto))
		active_tool = null
	else
...
/mob/living/silicon/drone/click(atom/target, params)
...
/mob/living/silicon/drone/Bump(atom/movable/AM as mob|obj, yes)
	spawn( 0 )
...
/mob/living/silicon/drone/say(var/message)
	if (!message)
		return
...
/mob/living/silicon/drone/emote(var/act)
	//var/param = null
...
/mob/living/silicon/drone/get_equipped_ore_scoop()
...
/mob/living/silicon/drone/proc/connect_to_drone(var/mob/living/L)
	if (!L || !src)
		return 1
	if (controller)
		return 2
...
/mob/living/silicon/drone/proc/disconnect_user()
	if (!controller)
		return
...
/obj/drone_frame/proc/change_stage(var/change_to,var/mob/user,var/obj/item/item_used)
	if (!isnum(change_to))
		return
	playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
	if (user && item_used)
...
/obj/drone_frame/attack_hand(var/mob/user as mob)
	switch(construct_stage)
		if(3)
...
/obj/drone_frame/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weldingtool))
		var/obj/item/weldingtool/WELD = W
		if (WELD.get_fuel() > 1)
			switch(construct_stage)
...
/obj/item/parts/robot_parts/drone/propulsion/New()
	..()
...
/obj/item/parts/robot_parts/drone/plating/New()
	..()
