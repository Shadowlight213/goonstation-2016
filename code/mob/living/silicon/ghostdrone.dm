...
/mob/living/silicon/ghostdrone/New()
	..()
	ghost_drones += src
...
/mob/living/silicon/ghostdrone/proc/updateStatic()
	if (!src.client)
		return
...
/mob/living/silicon/ghostdrone/death(gibbed)
...
/mob/living/silicon/ghostdrone/dispose()
	..()
...
/mob/living/silicon/ghostdrone/Del()
	if (src in ghost_drones)
		ghost_drones -= src
	if (src in available_ghostdrones)
		available_ghostdrones -= src
	...
	// so on with this mob
/mob/living/silicon/ghostdrone/updatehealth()
	return

/mob/living/silicon/ghostdrone/full_heal()
	var/before = src.stat
	..()
	if (before == 2 && src.stat < 2) //if we were dead, and now arent
		src.updateSprite()

/mob/living/silicon/ghostdrone/TakeDamage(zone, brute, burn)
...
/mob/living/silicon/ghostdrone/examine()
	..()
	var/msg = "*---------*<br>"
...
/mob/living/silicon/ghostdrone/Login()
	..()
	if (src.stat == 0)
		src.visible_message("<span style='color: blue'>[src.name] comes online.</span>", "<span style='color: blue'>You come online!</span>")
		src.updateSprite()

/mob/living/silicon/ghostdrone/Logout()
	..()
	src.updateSprite()

/mob/living/silicon/ghostdrone/proc/setStunnedEffects()
...
/mob/living/silicon/ghostdrone/proc/setFace(type = "happy", color = "#7fc5ed")
...
/mob/living/silicon/ghostdrone/proc/setFaceDialog()
...
/mob/living/silicon/ghostdrone/proc/updateHoverDiscs(color = "#7fc5ed")
...
/mob/living/silicon/ghostdrone/proc/updateSprite()
...
/mob/living/silicon/ghostdrone/hand_attack(atom/target, params)
...
/mob/living/silicon/ghostdrone/click(atom/target, params)
	if (params["shift"])
...
/mob/living/silicon/ghostdrone/Stat()
	..()
	if(src.cell)
...
/mob/living/silicon/ghostdrone/Bump(atom/movable/AM as mob|obj, yes)
	spawn( 0 )
...
/mob/living/silicon/ghostdrone/proc/putonHat(obj/item/clothing/head/W as obj, mob/user as mob)
	src.hat = W
	W.set_loc(src)
...
/mob/living/silicon/ghostdrone/proc/takeoffHat(forcedDir = null)
	UpdateOverlays(null, "hat")
...
/mob/living/silicon/ghostdrone/proc/putonSheet(obj/item/clothing/suit/bedsheet/W as obj, mob/user as mob)
	W.set_loc(src)
	src.bedsheet = W
	src.setFace(type = faceType, color = faceColor) //removes face overlay and lumin (also sets icon)
	return 1

/mob/living/silicon/ghostdrone/proc/takeoffSheet()
...
/mob/living/silicon/ghostdrone/attackby(obj/item/W as obj, mob/user as mob)
...
/mob/living/silicon/ghostdrone/attack_hand(mob/user)
	if(!user.stat)
...
/mob/living/silicon/ghostdrone/weapon_attack(atom/target, obj/item/W, reach, params)
...
/mob/living/silicon/ghostdrone/proc/store_active_tool()
	if (!src.active_tool)
		return
...
/mob/living/silicon/ghostdrone/equipped()
	if (!active_tool)
		return null
	return active_tool

/mob/living/silicon/ghostdrone/proc/uneq_slot()
	if (src.active_tool)
		if (istype(src.active_tool, /obj/item/magtractor) && src.active_tool:holding)
...
/mob/living/silicon/ghostdrone/proc/use_power()
	if (src.cell)
...
/mob/living/silicon/ghostdrone/Move(a, b, flag)
...
/mob/living/silicon/ghostdrone/drop_item_v()
	return

/mob/living/silicon/ghostdrone/emote(var/act, var/voluntary = 1)
	var/param = null
...
/mob/living/silicon/ghostdrone/proc/drone_broadcast(message)
	message = html_encode(src.say_quote(message))
...
/mob/living/silicon/ghostdrone/say(message = "")
...
/mob/living/silicon/ghostdrone/proc/show_laws_drone() //A new proc because it's handled very differently from normal laws
...
/mob/living/silicon/ghostdrone/verb/cmd_show_laws()
...
/mob/living/silicon/ghostdrone/bullet_act(var/obj/projectile/P)
...
/mob/living/silicon/ghostdrone/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	return

/mob/living/silicon/ghostdrone/canRideMailchutes()
	return 1

/mob/living/silicon/ghostdrone/restrained()
	return 0

/mob/living/silicon/ghostdrone/emag_act(var/mob/user, var/obj/item/card/emag/E)

/mob/living/silicon/ghostdrone/ex_act(severity)

/mob/living/silicon/ghostdrone/blob_act(var/power)

/mob/living/silicon/ghostdrone/emp_act()

/mob/living/silicon/ghostdrone/meteorhit(obj/O as obj)

/mob/living/silicon/ghostdrone/temperature_expose(null, temp, volume)
	if(src.material)
...
/mob/living/silicon/ghostdrone/get_static_image()
	return
...
/mob/living/silicon/ghostdrone/ghostize()
	if (src.client)
		src.client.images.Remove(mob_static_icons)
	..()