...
/mob/living/critter/proc/add_hh_flesh(var/min, var/max, var/mult)
...
/mob/living/critter/proc/add_hh_flesh_burn(var/min, var/max, var/mult)
...
/mob/living/critter/proc/add_hh_robot(var/min, var/max, var/mult)
...
/mob/living/critter/proc/add_hh_robot_burn(var/min, var/max, var/mult)
...
/mob/living/critter/proc/throw_item(atom/target)
	if (!can_throw)
		return
	src.throw_mode_off()
	if (usr.stat)
...
/mob/living/critter/find_in_hands(var/eqtype)
...
/mob/living/critter/hand_range_attack(atom/target, params)
...
/mob/living/critter/proc/melee_attack_human(var/mob/living/carbon/human/M, var/extra_damage) // non-special limb attack
	if (M.nodamage)
...
/mob/living/critter/attack_hand(var/mob/living/M)
	switch (M.a_intent)
		if (INTENT_HELP)
...
/mob/living/critter/proc/handle_hud_overlays()
	var/color_mod_r = 255
	var/color_mod_g = 255
	var/color_mod_b = 255
	if (src.druggy)
...
/mob/living/critter/death(var/gibbed)
	density = 0
	if (!gibbed)
...
/mob/living/critter/TakeDamage(zone, brute, burn)
	hit_twitch()
	if (nodamage)
		return
...
/mob/living/critter/proc/empty_hands()
	for (var/datum/handHolder/HH in hands)
		if (HH.item)
...
/mob/living/critter/update_burning_icon(var/old_burning)
	if (!burning)
...
/mob/living/critter/get_head_armor_modifier()
	var/armor_mod = 0
	for (var/datum/equipmentHolder/EH in equipment)
...
/mob/living/critter/get_chest_armor_modifier()
	var/armor_mod = 0
	for (var/datum/equipmentHolder/EH in equipment)
...
/mob/living/critter/get_explosion_resistance()
