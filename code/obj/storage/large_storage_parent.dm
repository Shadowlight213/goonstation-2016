...
/obj/storage/proc/make_my_stuff() // use this rather than overriding the container's New()
...
/obj/storage/alter_health()
	return get_turf(src)

/obj/storage/relaymove(mob/user as mob)
...
/obj/storage/attack_hand(mob/user as mob)
...
/obj/storage/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
...
/obj/storage/attack_ai(mob/user)
...
/obj/storage/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
...
/obj/storage/blob_act(power)
	if (prob(power * 2.5))
		dump_contents()
		qdel(src)

/obj/storage/meteorhit(obj/O as obj)
...
/obj/storage/proc/is_acceptable_content(atom/A)
...
/obj/storage/proc/can_open()
	if (src.welded || src.locked)
		return 0
	return 1

/obj/storage/proc/can_close()
	for (var/obj/storage/S in get_turf(src))
		if (S != src)
			return 0
	return 1

/obj/storage/proc/dump_contents()
...
/obj/storage/proc/toggle()
	if (src.open)
		return src.close()
	return src.open()

/obj/storage/proc/bust_out()
	if (src.health)
...
/obj/storage/proc/weld(shut = 0, obj/item/weldingtool/W as obj, mob/weldman as mob)
	if (shut)
		W.use_fuel(1)
		W.eyecheck(weldman)
...
/obj/storage/proc/crunch(mob/M as mob)
	...
	// Added (Convair880).
/obj/storage/proc/log_me(mob/user, mob/occupant, action = "")
...
/obj/storage/secure/update_icon()
	..()
	if (!src.open)
		src.icon_state = src.icon_closed
...
/obj/storage/secure/receive_signal(datum/signal/signal)
...
/obj/storage/secure/emag_act(mob/user, obj/item/card/emag/E)
...
/obj/storage/secure/demag(mob/user)
	if (!src.emagged)
		return 0
	else if (src.emagged)
		src.emagged = 0
