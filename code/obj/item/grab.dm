...
/obj/item/grab/attack(atom/target, mob/user)
	if (check())
		return
...
/obj/item/grab/attack_hand(mob/user)
	return
...
/obj/item/grab/proc/update_icon()
	switch (src.state)
		if (0)
			icon_state = "reinforce"
		if (1)
