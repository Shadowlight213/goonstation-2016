...
/obj/blob/proc/right_click_action()
	examine()

/obj/blob/Click(location, control, params)
	if (usr != overmind)
		return
...
/obj/blob/proc/setOvermind(var/mob/living/intangible/blob_overmind/O)
	if (overmind == O)
		return
	if (overmind)
		overmind.blobs -= src
...
/obj/blob/proc/onAttach(var/mob/living/intangible/blob_overmind/new_overmind)
	if (istype(new_overmind))
		if (spread_value)
			new_overmind.spread_mitigation += spread_value

/obj/blob/proc/attack(var/turf/T)
...
/obj/blob/bullet_act(var/obj/projectile/P)
...
/obj/blob/temperature_expose(datum/gas_mixture/air, temperature, volume)
...
/obj/blob/attack_hand(var/mob/user)
	var/adj1
...
/obj/blob/proc/create_chunk(var/turf/T)
...
/obj/blob/proc/take_damage(var/amount,var/damage_mult = 1,var/damtype = "brute",var/mob/user)
...
/obj/blob/proc/updatePoisonOverlay()
	if (!poison)
		animate(src)
...
/obj/blob/proc/heal_damage(var/amount)
...
/obj/blob/proc/spread(var/turf/T)
...
/obj/blob/proc/has_upgrade(var/upgrade_path)
...
/obj/blob/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	if (O == src)
		return
	if (O.disposed)
		return
...
/obj/blob/proc/onMove(var/obj/blob/B)
	return

/obj/blob/onMaterialChanged()
	..()
...
/obj/blob/nucleus/onAttach(var/mob/living/intangible/blob_overmind/O)
	..()
	O.nuclei += src

/obj/blob/nucleus/take_damage(amount, mult, damtype, mob/user)
...
/obj/blob/deposit/proc/update_reagent_overlay()
	if (disposed)
		return
...
/obj/blob/deposit/proc/build_reclaimer()
...
/obj/blob/deposit/proc/build_replicator()
	if (disposed || building)
		return
	building = 1
...
/obj/blob/deposit/onMove(var/obj/blob/B)
	var/turf/T = B.loc
	B.set_loc(loc)
	set_loc(T)
	update_icon()
...
/obj/blob/deposit/replicator/New()
	..()
	progress.owner = src
	progress.onStart()

/obj/blob/deposit/replicator/disposing()
	..()
	progress.onDelete()
	qdel(progress)

/obj/blob/deposit/replicator/proc/set_master_reagent()
...
/obj/blob/deposit/replicator/Life()
	if (converting)
...
/obj/blob/deposit/replicator/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	if (O.disposed)
		return
	if (overmind.tutorial)
		if (!overmind.tutorial.PerformAction("mousedrop", list(O, src)))
...
/obj/blob/deposit/replicator/onMove(var/obj/blob/B)
	if (B.disposed)
		return
	if (!isturf(B.loc))
		return
...
/obj/blob/deposit/reclaimer/Life()
	if (..())
		return 1
...
/obj/blob/launcher/New()
	..()
	if (!underlay_image)
		underlay_image = image('icons/mob/blob.dmi', "deposit-reagent")

/obj/blob/launcher/proc/update_reagent_underlay()
	if (disposed)
		return
	if (src.reagents.total_volume <= 0)
		underlays.len = 0
...
/obj/blob/launcher/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	if (O.disposed)
		return
	if (overmind.tutorial)
		if (!overmind.tutorial.PerformAction("mousedrop", list(O, src)))
...
/datum/projectile/slime/on_hit(atom/hit, angle, var/obj/projectile/O)
	..()
	if (O.reagents)
...
/obj/blob/reflective/bullet_act(var/obj/projectile/P)
...
/obj/blob/ectothermid/proc/consume(var/amt)
	if (amt <= 0)
		return 0
	while (amt)
...
/obj/blob/ectothermid/temperature_expose(datum/gas_mixture/air, temperature, volume)
	if (temperature > T20C)
		temperature -= consume(temperature - T20C)
	..(air, temperature, volume)

/obj/blob/ectothermid/Life()
	if (..())
		return 1
	points_used = 0
	damage_credit = 0
...
/obj/blob/lipid/onAttach(var/mob/living/intangible/blob_overmind/O)
	..()
	O.lipids += src

/obj/blob/lipid/proc/use()
	overmind.lipids -= src
...
/obj/blob/ribosome/onAttach(var/mob/living/intangible/blob_overmind/O)
	..()
	O.spread_mitigation += 1
	added = 1

/obj/blob/ribosome/update_icon()
	return

/obj/blob/ribosome/disposing()
	..()
	if (overmind)
...
/obj/blob/wall/take_damage(var/amount,var/damage_mult = 1,var/damtype,var/mob/user)
	if (damage_mult == 0)
		return
	if (damtype != "mixed")
...
/obj/blob/firewall/take_damage(amount, mult, damtype, mob/user)
	if (damtype == "burn")
		return
	else return ..()

/obj/blob/firewall/update_icon()
	return
...
/obj/material_deposit/New(nloc, mat, blob)
	..(nloc)
	src.overmind = blob
...
/obj/material_deposit/onMaterialChanged()
	pixel_x = rand(-12, 12)
	pixel_y = rand(-12, 12)

/obj/material_deposit/attackby(var/obj/item/W, var/mob/user)
