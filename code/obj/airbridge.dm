...
/obj/airbridge_controler/New()
	airbridge_controllers += src
	..()

/obj/airbridge_controler/proc/get_link()
	for(var/obj/airbridge_controler/C in world)
		if(C.z == src.z && C.id == src.id && C != src)
			linked = C
			break

/obj/airbridge_controler/proc/toggle_bridge()
...
/obj/airbridge_controler/proc/pressurize()
	if(linked == null) get_link()
	if(linked == null) return
...
/obj/airbridge_controler/proc/get_state_string()
	if(linked == null) get_link()
...
/obj/airbridge_controler/proc/is_working()
	if(linked == null) get_link()
...
/obj/airbridge_controler/proc/establish_bridge()
	if(linked == null) get_link()
	if(linked == null) return
...
/obj/airbridge_controler/proc/remove_bridge()
	if(linked == null) get_link()
	if(linked == null) return
...
/obj/machinery/computer/airbr/process()
	..()
	update_status()
...
/obj/machinery/computer/airbr/proc/pick_controller()
...
/obj/machinery/computer/airbr/proc/do_initial_extend()
...
/obj/machinery/computer/airbr/proc/update_status()
	if (!links.len)
		get_links()
...
/obj/machinery/computer/airbr/attack_hand(var/mob/user as mob)
	if (..(user))
		return
...
/obj/machinery/computer/airbr/proc/ensure_links()
	if (!src.links.len)
		src.get_links()
	if (!src.links.len)
		src.working = 0
...
/obj/machinery/computer/airbr/proc/establish_bridge()
	if (!src.ensure_links())
		return 0
	var/obj/airbridge_controler/C = src.pick_controller()
	if (istype(C))
...
/obj/machinery/computer/airbr/proc/remove_bridge()
	if (!src.ensure_links())
		return 0
	var/obj/airbridge_controler/C = src.pick_controller()
	if (istype(C))
...
/obj/machinery/computer/airbr/proc/pressurize()
	if (!src.ensure_links())
		return 0
	var/obj/airbridge_controler/C = src.pick_controller()
	if (istype(C))
...
/obj/machinery/computer/airbr/Topic(href, href_list)
	if (..(href, href_list))
		return
...
/obj/machinery/computer/airbr/power_change()
	if(stat & BROKEN)
		icon_state = "airbrbr"
		light.disable()
		...
		spawn(rand(0, 15))
			...
			stat |= NOPOWER
			light.disable()
/obj/machinery/computer/airbr/set_broken()
...
/obj/machinery/airbr_test_button/attack_hand(mob/user as mob)
...
/obj/shuttledummy/proc/slide(var/direction)
	switch(direction)
		if(NORTH)
			while(pixel_y < 32)
				pixel_y += 4
