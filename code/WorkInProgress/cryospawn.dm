...
/obj/cryotron/proc/add_person_to_queue(var/mob/living/person)
	if (!istype(person))
		return 0
...
#ifdef DESTINY_MODE
/obj/cryotron/proc/process()
	spawn_next_person()
	ensure_storage()
#endif

	//Return 1 if there is another person to spawn afterward
/obj/cryotron/proc/spawn_next_person()
...
#ifdef DESTINY_MODE
/obj/cryotron/proc/add_person_to_storage(var/mob/living/L as mob)
	if (!istype(L))
		return 0
...
/obj/cryotron/proc/enter_prompt(var/mob/living/user as mob)
...
/obj/cryotron/proc/mob_can_enter_storage(var/mob/living/L as mob)
	if (!ticker)
...
/obj/cryotron/proc/exit_prompt(var/mob/living/user as mob)
...
/obj/cryotron/proc/ensure_storage()
	if (!stored_mobs.len)
		return
...
/obj/cryotron/attack_hand(var/mob/user as mob)
	if (!enter_prompt(user))
		return ..()

/obj/cryotron/attackby(var/obj/item/W as obj, var/mob/user as mob)
	if (!enter_prompt(user))
		return ..()

/obj/cryotron/relaymove(var/mob/user as mob, dir)
