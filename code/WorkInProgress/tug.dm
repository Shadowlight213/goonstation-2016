...
/obj/tug_cart/MouseDrop_T(atom/movable/C, mob/user)
...
/obj/tug_cart/MouseDrop(obj/over_object as obj, src_location, over_location)
	..()
...
/obj/tug_cart/proc/load(atom/movable/C)
...
/obj/tug_cart/proc/unload(turf/T)//var/dirn = 0)
	if (!load)
		return
	if (!isturf(T))
		T = get_turf(T)
...
/obj/vehicle/tug/eject_rider(crashed, selfdismount)
...
/obj/vehicle/tug/relaymove(mob/user as mob, dir)
	if (rider)
...
/obj/vehicle/tug/MouseDrop_T(atom/movable/C, mob/user)
	if (!in_range(user, src) || !in_range(user, C) || user.restrained() || user.paralysis || user.sleeping || user.stat || user.lying)
		return
...
/obj/vehicle/tug/attack_hand(mob/living/carbon/human/M as mob)
	if (!M || !rider)
		..()
		return
	switch (M.a_intent)
...
/obj/vehicle/tug/bullet_act(flag, A as obj)
	if (rider)
		eject_rider()
		rider.bullet_act(flag, A)
	return

/obj/vehicle/tug/meteorhit()
	if (rider)
		eject_rider()
		rider.meteorhit()
	return

/obj/vehicle/tug/disposing()
	if (rider)
