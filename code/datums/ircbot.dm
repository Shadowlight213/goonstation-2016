...
/datum/ircbot/proc/event(type, data)
	if (!type) return 0
...
/datum/ircbot/proc/export(iface, args)
	if (src.debugging)
...
/datum/ircbot/proc/response(args)
	if (src.debugging)
...
/datum/ircbot/proc/toggleDebug(client/C)
	if (!C) return 0
...
/datum/ircbot/proc/logDebug(log)
	if (!log) return 0
